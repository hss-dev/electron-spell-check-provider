...
_.extend(SpellCheckProvider.prototype, {
  spellCheck(words, callback) {
    setTimeout(() => {
      const misspelled = words.filter(x => !this.getMisspelled(x));
      callback(misspelled);
    }, 0);
  },
  getMisspelled: function(element) {
    if (element !== "" && !this.correct(element)) {
      if (last.has(element)) {
        this.emit("misspelling", last.get(element));
        return false;
      }
      const suggestions = _nspellInstance.suggest(element);
